            ...
            <ul className='list-none mt-4'>
              {
                item.links.map((itemLink, index) => ( 
                  <li key={itemLink.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== itemLink.length ? ' mb-4' : 'mb-0'}`}>
                    {itemLink.name}
                  </li>
                ))
              }
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
        {
          socialMedia.map((socialIcon, index) => ( 
            ...
            src={socialIcon.icon} 
            alt={socialIcon.id}
            className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialIcon.length - 1 ? 'mr-6' : 'mr-0'}`}
            />
        ))
        }
      </div>
